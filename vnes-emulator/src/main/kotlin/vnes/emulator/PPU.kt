 ...
 */
class PPU(private var nes: NES?) : PPUCycles {
    ...
    private var tpix: IntArray = IntArray(8)
    ...
    fun startVBlank() {
        ...
        nes!!.cpu!!.requestIrq(CPU.Companion.IRQ_NMI)

        // Make sure everything is rendered:
        if (lastRenderedScanline < 239) {
            renderFramePartially(
                nes!!.gui!!.getScreenView().getBuffer(),
                lastRenderedScanline + 1,
        ...
        val frameBuffer = nes!!.gui!!.getScreenView().getBuffer()
        ...
        // Notify image buffer:
        nes!!.gui!!.getScreenView().imageReady(false)
    ...
    fun endScanline() {
        if (scanline < 19 + vblankAdd) {
            ...
            if (f_bgVisibility == 1 || f_spVisibility == 1) {
                ...
                nes!!.memoryMapper!!.clockIrqCounter()
            }
    ...
    fun startFrame() {
        val buffer = nes!!.gui!!.getScreenView().getBuffer()
    ...
    fun endFrame() {
        ...
        // Show sound buffer:
        if (showSoundBuffer && nes!!.papu!!.getLine() != null) {
            bufferSize = nes!!.papu!!.getLine().getBufferSize()
            available = nes!!.papu!!.getLine().available()
    ...
    fun setStatusFlag(flag: Int, value: Boolean) {
        val n = 1 shl flag
        var memValue = nes!!.cpuMemory!!.load(0x2002).toInt()
        memValue = ((memValue and (255 - n)) or (if (value) n else 0))
        nes!!.cpuMemory!!.write(0x2002, memValue.toShort())
    }
    ...
    fun writeVRAMAddress(address: Int) {
        ...
        if (vramAddress < 0x2000) {
            nes!!.memoryMapper!!.latchAccess(vramAddress)
        }
    }
    ...
    private fun renderFramePartially(buffer: IntArray?, startScan: Int, scanCount: Int) {
        ...
        if (nes!!.isNonHWScalingEnabled && !this.isRequestRenderAll) {
    ...
    fun updatePalettes() {
        for (i in 0..15) {
            if (f_dispType == 0) {
                imgPalette[i] = nes!!.palTable!!.getEntry(ppuMem!!.load(0x3f00 + i).toInt() and 63)
            } else {
                imgPalette[i] = nes!!.palTable!!.getEntry(ppuMem!!.load(0x3f00 + i).toInt() and 32)
            }
        }
        for (i in 0..15) {
            if (f_dispType == 0) {
                sprPalette[i] = nes!!.palTable!!.getEntry(ppuMem!!.load(0x3f10 + i).toInt() and 63)
            } else {
                sprPalette[i] = nes!!.palTable!!.getEntry(ppuMem!!.load(0x3f10 + i).toInt() and 32)
            }
        }
