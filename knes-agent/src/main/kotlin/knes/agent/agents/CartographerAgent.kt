 ...
 *
 * Budget: time + vision-call cap from Config. If exceeded, falls back to
...
class CartographerAgent(
    ...
    suspend fun exploreInitialOverworld() {
        ...
        knes.agent.runtime.Log.cartographer("start: budget=${budgetSeconds}s maxCalls=$maxVisionCalls")

        while (true) {
            val elapsed = (System.currentTimeMillis() - started) / 1000
            if (elapsed > budgetSeconds) { knes.agent.runtime.Log.cartographer("time budget exceeded (${elapsed}s)"); break }
            ...
            if (fog.allReachableKnown(worldX to worldY)) {
                knes.agent.runtime.Log.cartographer("frontier exhausted at ($worldX,$worldY)")
                break
            }
            ...
            when (direction.uppercase()) {
                ...
                "DONE" -> break
                else -> knes.agent.runtime.Log.warn("cartographer unexpected dir: $direction")
            }
        ...
        knes.agent.runtime.Log.cartographer("done: $visionCalls vision calls, $turnCounter steps")
        run?.markIdle()
    }

    suspend fun targetedRepass(flags: List<String>) {
        knes.agent.runtime.Log.cartographer("targetedRepass: ${flags.size} flags (stub)")
    }
