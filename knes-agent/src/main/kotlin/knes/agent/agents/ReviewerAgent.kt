...
class ReviewerAgent(
    ...
    suspend fun auditPlan(turn: Int, screenshotB64: String, ramDigest: String): List<String> {
        ...
        val raw = try {
            ...
            knes.agent.runtime.Log.error("auditPlan LLM error: ${e.message?.take(120)}", turn)
            run?.markIdle()
            return emptyList()
        }
        runCatching {
        ...
        val parsed = try {
            ...
            knes.agent.runtime.Log.error("auditPlan parse error: ${e.message?.take(80)} raw=${raw.take(120)}", turn)
            run?.markIdle()
            return emptyList()
        }
        val issues = parsed.issues.map { "step=${it.step}: ${it.problem.take(80)}" }
        if (issues.isNotEmpty()) {
            knes.agent.runtime.Log.reviewer("AUDIT found ${issues.size} issues: $issues", turn)
    ...
    suspend fun audit(turn: Int) {
        ...
        memory.saveCampaign()
        knes.agent.runtime.Log.reviewer("memory-audit (placeholder, no LLM)", turn)
        run?.markIdle()
    }
    ...
    fun verifyMilestones(phase: Phase, ram: Map<String, Int>, turn: Int): List<String> {
        ...
        if (checked.isNotEmpty()) {
            knes.agent.runtime.Log.reviewer("verify checked=[${checked.joinToString(",")}] regressed=[${regressed.joinToString(",")}]", turn)
        }
        run?.markIdle()
        return regressed
    }
    ...
    fun checkProgress(turn: Int, ram: Map<String, Int>): StuckDiagnostic? {
        ...
        knes.agent.runtime.Log.warn("Reviewer.checkProgress: ${current.id} stuck $elapsed turns — triggering replan", turn)
    ...
    private fun applyRemove(entry: String) {
        ...
        knes.agent.runtime.Log.reviewer("would remove: $entry")
    }
